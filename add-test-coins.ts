...
async function addTestCoins() {
  ...
  try {
    ...
    if (!user) {
      ...
      allUsers.forEach(u => {
        console.log(`  - ${u.id} (${u.name}) - ${(u.coins || new Decimal(0)).toFixed(2)} coins`);
      });
      ...
      if (allUsers.length > 0) {
        ...
        console.log('✅ New balance for', firstUser.name, ':', (result.coins || new Decimal(0)).toFixed(2));
      }
    ...
    console.log('✅ New balance for', user.name, ':', (result.coins || new Decimal(0)).toFixed(2));
  } catch (error) {
