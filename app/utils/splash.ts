...
const splash = {
  ...
  render() {
    ...
    if (this.n >= 1) {
      setTimeout(() => this.end(), 300); // Reduced from 1000ms to 300ms
    } else {
      progress.style.width = this.n * 100 + "%";
      setTimeout(() => this.render(), 200); // Reduced from 500ms to 200ms
    }
  },
    ...
    setTimeout(() => (splash.style.display = "none"), 300); // Reduced from 1000ms to 300ms
  }
};
