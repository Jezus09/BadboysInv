...
export async function acceptTrade(tradeId: string) {
  ...
  console.log("Sender items to trade:", senderItems.length, senderItems.map(i => `${i.id}(uid:${i.uid})`));
  console.log("Receiver items to trade:", receiverItems.length, receiverItems.map(i => `${i.id}(uid:${i.uid})`));
  ...
  try {
    ...
    console.log("Original sender inventory UIDs:", Object.keys(senderInventoryData.items));
    console.log("Original receiver inventory UIDs:", Object.keys(receiverInventoryData.items));
    ...
    for (const item of senderItems) {
      ...
      for (const [uid, invItem] of Object.entries(newSenderInventory.items)) {
        const match = invItem.id === item.id && 
      ...
      if (!found) {
        throw new Error(`Trade failed: Sender no longer has item ID=${item.id} (wear=${item.wear}, nameTag=${item.nameTag})`);
      }
    }
    ...
    for (const item of receiverItems) {
      ...
      if (!found) {
        throw new Error(`Trade failed: Receiver no longer has item ID=${item.id} (wear=${item.wear}, nameTag=${item.nameTag})`);
      }
    }
    ...
    for (const item of senderItems) {
      console.log(`Looking for sender item to remove: ID=${item.id}, wear=${item.wear}, nameTag=${item.nameTag}`);
      let found = false;
      ...
      if (!found) {
        ...
        throw new Error(`CRITICAL ERROR: Could not find sender item ID=${item.id} after validation`);
      }
    }
    
    // Remove receiver items from receiver inventory (by item properties, not UID)
    for (const item of receiverItems) {
      console.log(`Looking for receiver item to remove: ID=${item.id}, wear=${item.wear}, nameTag=${item.nameTag}`);
      let found = false;
      ...
      for (const [uid, invItem] of Object.entries(newReceiverInventory.items)) {
        ...
        if (match) {
          console.log(`Found matching receiver item at UID ${uid}, removing it`);
      ...
      if (!found) {
        // This should never happen now since we validated above
        throw new Error(`CRITICAL ERROR: Could not find receiver item ID=${item.id} after validation`);
      }
    }
    
    // Find next available UIDs
    const getNextUID = (inventory: any) => {
      const existingUIDs = Object.keys(inventory.items).map(k => parseInt(k)).filter(n => !isNaN(n));
    ...
    console.log("New sender inventory UIDs:", Object.keys(newSenderInventory.items));
    console.log("New receiver inventory UIDs:", Object.keys(newReceiverInventory.items));
    ...
    if (senderCoins.gt(0)) {
      ...
      if (senderBalance.lt(senderCoins)) {
        throw new Error(`Trade failed: Sender has insufficient coins. Required: ${senderCoins}, Available: ${senderBalance}`);
      }
    }
    ...
    if (receiverCoins.gt(0)) {
      ...
      if (receiverBalance.lt(receiverCoins)) {
        throw new Error(`Trade failed: Receiver has insufficient coins. Required: ${receiverCoins}, Available: ${receiverBalance}`);
      }
    }
    ...
    if (senderCoins.gt(0)) {
      ...
      const subtractResult = await subtractCoins(trade.senderUserId, senderCoins, "trade_out", `Trade ${tradeId} - coins sent`);
      if (!subtractResult.success) {
        throw new Error(`Coin transfer failed: ${subtractResult.error}`);
      }
      await addCoins(trade.receiverUserId, senderCoins, "trade_in", `Trade ${tradeId} - coins received`);
    }
    
    if (receiverCoins.gt(0)) {
      console.log(`Transferring ${receiverCoins} coins from receiver to sender`);
      const subtractResult = await subtractCoins(trade.receiverUserId, receiverCoins, "trade_out", `Trade ${tradeId} - coins sent`);
      if (!subtractResult.success) {
        throw new Error(`Coin transfer failed: ${subtractResult.error}`);
      }
      await addCoins(trade.senderUserId, receiverCoins, "trade_in", `Trade ${tradeId} - coins received`);
    }
    ...
    try {
      await Promise.all([
        ...
        manipulateUserInventory({
          ...
          async manipulate(inventory) {
            // Just sync, no changes needed - the inventory was already updated in DB
            console.log(`Synced receiver inventory - ${inventory.size()} items`);
          }
        })
      ]);
...
export async function getTradeHistory(userId: string, page: number = 1, limit: number = 10) {
