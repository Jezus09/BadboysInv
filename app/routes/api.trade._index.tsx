...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    switch (action) {
      ...
      case "get":
        if (!tradeId) {
          return data({ success: false, message: "Trade ID required" }, { status: 400 });
        }
        const trade = await getTrade(tradeId);
        if (!trade) {
          return data({ success: false, message: "Trade not found" }, { status: 404 });
        }
        // Check if user is part of this trade
        if (trade.senderUserId !== user.id && trade.receiverUserId !== user.id) {
      ...
      case "search-users":
        if (!searchQuery || searchQuery.length < 2) {
          return data({ success: false, message: "Search query too short" }, { status: 400 });
        }
      ...
      case "get-user-inventory":
        ...
        if (!targetUserId) {
          return data({ success: false, message: "User ID required" }, { status: 400 });
        }
        ...
        console.log("Trade API - Raw inventory string:", inventoryString?.substring(0, 200) + "...");
        ...
        console.log("Trade API - Parsed inventory data:", parsedInventoryData ? "valid" : "invalid");
        ...
        if (parsedInventoryData && parsedInventoryData.items) {
          ...
          console.log("Trade API - Using official inventory format, items count:", inventory.length);
        } else {
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    if (!action) {
      return data({ success: false, message: "No action specified" }, { status: 400 });
    }
    ...
    switch (String(action)) {
      case "create": {
        ...
        if (!receiverUserId || !senderItemsJson) {
          return data({ success: false, message: "Missing required fields" }, { status: 400 });
        }
        ...
        // Validate items
        const validatedSenderItems = z.array(tradeItemSchema).parse(senderItems);
        const validatedReceiverItems = z.array(tradeItemSchema).parse(receiverItems);
      ...
      case "accept": {
        ...
        if (trade.receiverUserId !== user.id) {
          return data({ success: false, message: "Only the receiver can accept this trade" }, { status: 403 });
        }

        if (trade.status !== "PENDING") {
          return data({ success: false, message: "Trade is no longer pending" }, { status: 400 });
        }
      ...
      case "decline": {
        ...
        if (trade.receiverUserId !== user.id) {
          return data({ success: false, message: "Only the receiver can decline this trade" }, { status: 403 });
        }
      ...
      case "cancel": {
        ...
        if (trade.senderUserId !== user.id) {
          return data({ success: false, message: "Only the sender can cancel this trade" }, { status: 403 });
        }
