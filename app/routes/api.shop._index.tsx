...
import { getShopItems, getShopItem, getShopItemsByCategory } from "~/models/shop.server";
import { subtractCoins } from "~/models/currency.server";
import { getUserInventory, manipulateUserInventory } from "~/models/user.server";
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (action === "purchase") {
    if (!shopItemId) {
      return data({ success: false, message: "Shop item ID is required" }, { status: 400 });
    }
    ...
    try {
      ...
      if (!shopItem) {
        return data({ success: false, message: "Shop item not found" }, { status: 404 });
      }
