...
async function restoreDatabase(backupFile?: string) {
  ...
  try {
    ...
    console.log(
      `   - Shop itemek: ${backupData.counts?.shopItems || backupData.shopItems?.length || 0}`
    );
    console.log(
      `   - Rules: ${backupData.counts?.rules || backupData.rules?.length || 0}`
    );
    console.log(
      `   - Users: ${backupData.counts?.users || backupData.users?.length || 0}`
    );
    ...
    console.log(
      "👥 Users backup-olva van, de nem állítjuk vissza automatikusan"
    );
