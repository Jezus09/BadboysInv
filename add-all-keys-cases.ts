...
async function addAllKeysAndCases() {
  ...
  try {
    ...
    const allContainers = CS2Economy.filterItems({ type: CS2ItemType.Container });
    
    // Categorize containers
    const weaponCases = allContainers.filter(item => item.isWeaponCase());
    const stickerCapsules = allContainers.filter(item => item.isStickerCapsule());
    ...
    const otherContainers = allContainers.filter(item => 
    ...
    const totalContainers = weaponCases.length + stickerCapsules.length + 
                           graffitiBoxes.length + souvenirCases.length + otherContainers.length;
